...
import UModal from '../../../../components/tag/TagModal';
import AlertTagDelete from '../../../../components/tag/AlertTagDelete';
...
function ReactTable({ data, columns, modalToggler, TagsLoading, refreshTags }) {
  const theme = useTheme();
  const [sorting, setSorting] = useState([{ id: 'tag_name', desc: false }]);
        ...
        <Stack direction="row" alignItems="center" spacing={2}>
          ...
          {TagsLoading ? (
            <AnimateButton type="rotate">
              <TooltipIconButton title="Reloading Tag..." color="warning" icon={<IconSettings />} />
            </AnimateButton>
          ) : (
            <TooltipIconButton title="Reload Tag" color="success" icon={<IconRefresh color="blue" />} onClick={refreshTags} />
          )}
          <CSVExport {...{ data: table.getSelectedRowModel().flatRows.map((row) => row.original), headers, filename: 'Tag-list.csv' }} />
        </Stack>
      </Stack>
      <ScrollX>
        <Stack>
...
export default function TagListPage() {
  const theme = useTheme();
  const { TagsLoading: loading, tags: lists, TagsLoading, refreshTags } = useGetTags();
              ...
              <Tooltip title="Delete">
                <IconButton
                  ...
                  onClick={(e) => {
                    e.stopPropagation();
                    handleClose();
                    setTagDeleteId(row.original.tag_id);
                  }}
                >
                  <IconTrash />
                </IconButton>
  ...
  return (
    <>
      ...
      />
      <AlertTagDelete id={Number(TagDeleteId)} title={TagDeleteId} open={open} handleClose={handleClose} />
      <UModal open={TagModal} modalToggler={setTagModal} Tag={selectedTag} />
    </>
  );
}
