...
import { updatePortfolio, useGetPortfolioInitialData } from '../../../../api/portfolio';
import { useGetBlogInitialData } from '../../../../api/blog';
import MainCard from '../../../../ui-component/MainCard';

const EditPortfolioPage = () => {
  const { post_id } = useParams(); // Get the portfolio_id from the URL params
  ...
  // Fetch portfolio data to edit
  const { data } = useGetPortfolioInitialData(post_id);
  ...
  useEffect(() => {
    setForm({
      ...
      status: data.status,
      portfolio_image: null,
      technologies: parseTechnologies(data.technologies), // Parse safely
  ...
  // Helper function to safely parse technologies (string or array)
  const parseTechnologies = (tech) => {
    if (Array.isArray(tech)) return tech; // Already an array
    if (typeof tech === 'string') {
      try {
        return JSON.parse(tech); // Parse stringified array
      } catch (e) {
        return []; // Fallback if parsing fails
      }
    }
    return []; // Fallback if not string or array
  };

  const handleSubmit = async () => {
    const requiredFields = ['title', 'description', 'category_id', 'technologies'];
    ...
    try {
      setLoading(true);
      const response = await updatePortfolio(post_id, formData);
  ...
  return (
    <MainCard title="Edit Portfolio">
      <Grid container spacing={2}>
        ...
        <Grid item xs={12} sm={6}>
          <TextField fullWidth label="Description" name="description" value={form.description} onChange={handleChange} multiline rows={4} />
        </Grid>
        <Grid item xs={12} sm={6}>
          <Autocomplete
            multiple
            ...
            renderTags={(tagValue, getTagProps) =>
              tagValue?.map((option, index) => (
                <Chip variant="outlined" label={option?.tag_name} {...getTagProps({ index })} key={option?.tag_id || index} />
              )) || []
            }
        ...
        <Grid item xs={12} sm={6}>
          <TextField fullWidth label="Budget" name="budget" value={form.budget} onChange={handleChange} type="number" />
        </Grid>
        ...
        <Grid item xs={12} sm={6}>
          <Autocomplete
            ...
            options={[]} // This can remain empty for freeSolo
            value={Array.isArray(form.technologies) ? form.technologies : []}
            onChange={(event, newValue) => {
              setForm((prev) => ({
                ...prev,
                technologies: Array.isArray(newValue) ? newValue : []
              }));
            }}
            ...
            renderInput={(params) => (
              <TextField {...params} variant="outlined" label="Technologies *" placeholder="Enter technology and press Enter" />
            )}
          />
        </Grid>
